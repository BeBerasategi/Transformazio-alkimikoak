...
OUTPUT_DIR = "outputs"
OUTPUT_SIMULATION = ["outputs_NVT", "outputs_NPT", "outputs_NVTnp"]
...
molecule, molecule_pdb, results_file = load_molecule(RESULTS_PATH_DICT[OUTPUT_SIMULATION[0]], molecule_name)
...
emaitzen_tentsorea_gorde(molecule_name, "solvated", U_kln, RESULTS_PATH)
...
U_kln = transformazio_alkimikoa_simulatu(simulation_vacuum, vacuum_model, alchemical_state_vacuum, EQUILIBRATION_PATH_DICT[OUTPUT_SIMULATION[0]]+'/vacuum')
emaitzen_tentsorea_gorde(molecule_name, "vacuum", U_kln, RESULTS_PATH)
print("Vacuum system calculated and saved.")
...
print(f"Solvatation free energy: {deltaG} = {deltaG_cal}")
print(f"Solvatation free energy: {deltaG} = {deltaG_cal}", file= results_file)
