...
# Call it this way: python3 alchemical_osoa_NPT.py -m 'cyclopentanol'
# NPT multzoa erabiltzen saiatzen naizen lehenengo aldia!
...
def transformazio_alkimikoa_simulatu(simulation, model, alchemical_state, equilibration_path):
    # NPT multzoko simulazioa egiteko beharrezko parametroak:
    ...
    for k in range(0,n_lambda): 
        # NVT-ra itzuli:
        if k>0 :
            for i in range(simulation_solvated.system.getNumForces()):
                if simulation_solvated.system.getForce(i).getName() == 'MonteCarloBarostat':
                    simulation_solvated.system.removeForce(i)
                    break
            ...
            #for force in l[::-1]:
            #    simulation_solvated.system.removeForce(force)
        ...
        # Sistema orekatu, NVT multzoan   
        report_interval = 100   # 100 pausuero (4fs * 100 = 400fs) neurketa egin.
        equilibration_file_NVT = os.path.join(equilibration_path, f"NVT_Equilibration_Data_{k}.dat")
        data_reporter_NVT = app.StateDataReporter(file=equilibration_file_NVT, reportInterval=report_interval, potentialEnergy=True, volume=True)
        simulation.reporters.append(data_reporter_NVT)
        # 50ps-ko (4fs * 5000 = 20ps) simulazioa.
        simulation.step(5000)

        print(f"{k}. bikotea NVT-en orekatuta", flush=True)
        simulation.reporters.pop(0)
        equilibration_figure = os.path.join(equilibration_path, f"NVT_Equilibration_Data_{k}.png")
        plot_energy(equilibration_figure, equilibration_file_NVT, report_interval)
        ...
        print(f"{k}. bikotea NPT-en orekatuta", flush=True)
        simulation.reporters.pop(0)
        ...
        with open(npt_stats_file, 'a') as stats_file:
            ...
            print(npt_data.std(axis=0), file=stats_file)
