...
PROJECT_ROOT_DIR = "."
OUTPUT_DIR = "outputs_NVT_non_periodic"
...
# Solvation: NPT hartuta.
RESULTS_PATH_NPT = os.path.join("./outputs_NPT", molecule_name,"result_files")
U_kln = np.load(RESULTS_PATH_NPT+f'/measurements_solvated_{molecule_name}.npy')
...
print(f"Solvatation free energy (NPT, NVT np): {deltaG} = {deltaG_cal}", flush=True)
print(f"Solvatation free energy (NPT, NVT np): {deltaG} = {deltaG_cal}", file= results_file, flush=True)
